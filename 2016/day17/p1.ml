...
let rec find_path () =
  if Queue.is_empty frontier then failwith "Cannot reach bottom-right room"
  else begin
    let cur_state = Queue.take frontier in
    if cur_state.current_room = end_room then cur_state
    else begin
...
let _ =
  ...
  print_string @@ "Path to reach the vault: ";
  print_string end_state.path;
  print_endline ""
