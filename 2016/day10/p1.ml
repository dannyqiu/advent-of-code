...
let rec give_value_to_target state v t =
  ...
  | Output o -> begin
      let old_output_values = (
        try
          let values = Hashtbl.find state.outputs o in
...
let rec run_simulation state =
  ...
  | bot::t -> begin
      ...
      match values_held with
      | ((Some v1, t1), (Some v2, t2)) ->
        if v1 = 17 && v2 = 61 then begin
          state.answer_bot <- bot;
          state
        end
        else begin
...
let _ =
  ...
  print_string "Bot that compares value-61 and value-17 microchips: ";
  print_int end_state.answer_bot;
  print_endline ""
