...
let apply_op op password =
  ...
  | ROTATE_POSITION (x) -> begin
      ...
      rotate password (i + 1 + (if i >= 4 then 1 else 0))
    end
  ...
  | MOVE (x, y) -> begin
      let first_temp = Str.string_before password x in
      let sx = String.sub password x 1 in
      let last_temp = Str.string_after password (x + 1) in
      let temp = first_temp ^ last_temp in
      let first = Str.string_before temp y in
      let last = Str.string_after temp y in
      first ^ sx ^ last
    end

let rec scramble_password state =
  match state.operations with
  | op::t -> scramble_password { operations = t;
...
let rec handle_input state lines =
  ...
  | _ -> scramble_password { state with operations = List.rev state.operations }
...
let _ =
  ...
  print_string "The scrambled password is: ";
