...
let run_registers = [| ref 0; ref 0; ref 0; ref 0 |]
...
let parse_out args =
  match args with
  | r when is_reg r -> OUTR(get_reg r)
  | _ -> OUTV(args |> int_of_string)
...
let rec program lineno run_state output cycles =
  if lineno >= Array.length run_state.program || cycles <= 0
  then run_state
  else begin
    ...
    let () = (
      ...
      | OUTR r         -> output := (!r)::(!output)
      | OUTV v         -> output := v::(!output)
      | TGL instr -> begin
          match instr with
          ...
          | TGLR r         -> incr r
          | TGLV v         -> ()
          | OUTR r         -> failwith "Day 25 does not require this!"
          | OUTV v         -> failwith "Day 25 does not require this!"
          | _ -> failwith "TGL cannot be encapsulated by itself!"
        end
    ) in
    program !nextlineno run_state output (cycles - 1)
  end

let brute_force run_state =
  let check_output output = List.length output > 100
                            && output
                               |> List.rev
                               |> List.mapi (fun i s -> s = (i mod 2))
                               |> List.for_all (fun x -> x)
  in
  let start_a = ref 0 in
  let found_value = ref false in
  while not !found_value do
    begin
      run_state.registers.(0) := !start_a;
      run_state.registers.(1) := 0;
      run_state.registers.(2) := 0;
      run_state.registers.(3) := 0;
      let output = ref [] in
      let _ = program 0 run_state output 1000000 in
      if check_output !output then found_value := true else incr start_a
    end
  done;
  run_state.registers.(0) := !start_a;
  run_state
...
let _ =
  ...
  print_string "Value in register a to initialize clock signal: ";
