...
function processData() {
    ...
    var best = getBestScore(ingredients);
    console.log("The best score of a cookie is: " + best);
}

function getBestScore(ingredients) {
    var ingredientNames = Object.keys(ingredients);
    var quantity = [];
    return getBestScoreRecursive(ingredients, ingredientNames, quantity);
}

function getBestScoreRecursive(ingredients, ingredientNames, quantity) {
    ...
    if (quantity.length == ingredientNames.length-1) {
        ...
        var score = calculateScore(ingredients, ingredientNames, quantity);
        quantity.pop();
        return score;
    }
    // otherwise, we will loop through all the possible quantities of the middle ingredients
        ...
        for (var i = 0; i < remainingQuantity; ++i) {
            quantity.push(i);
            var score = getBestScoreRecursive(ingredients, ingredientNames, quantity);
            if (score > bestScore) {
                bestScore = score;
            }
            quantity.pop();
