...
function processData() {
    ...
    var gold = maxGoldSpent(store, player, boss);
    console.log("The most amount of gold spent to lose the fight: " + gold);
}

function maxGoldSpent(store, player, boss) {
    var items = []
    return maxGoldSpentRecursive(store, player, boss, items);
}

function maxGoldSpentRecursive(store, player, boss, items) {
    if (items.length == 4) {
        ...
        if (isPlayerWin(playerStats, boss)) {
            return -999999; // if player wins, we disregard this configuration
        }
        else {
        ...
        var cost = Number.MIN_SAFE_INTEGER;
        if (items.length == 0) { // must buy at least one weapon
            for (var i = 0; i < 5; ++i) {
                items.push(i);
                cost = Math.max(cost, maxGoldSpentRecursive(store, player, boss, items));
                items.pop();
            }
            return cost;
        }
        ...
        cost = maxGoldSpentRecursive(store, player, boss, items);
        items.pop();
